...
## 1. Kaprekar numbers
![](/resources/8-03-surfing-chaos/D._R._Kaprekar.gif){image align="right"}
In the previous two Parallelograms we looked at discovered made by D.R. Kaprekar, an Indian mathematics teacher who was born in 1905.
...
This week, we’re going to look at something called a Kaprekar number, which Wikipedia defines as:

_“a positive integer with the property that if it is squared, then its representation can be partitioned [or split] into two positive integer parts whose sum is equal to the original number (e.g. 45, since 45<sup>2</sup> = 2025, and 20 + 25 = 45”._
...
When you square each of these numbers, you can then partition or split them into two numbers before adding them to make the original number. When you do this, the value of the smaller number is always….
...
Usually, if two systems start off in ALMOST the same way, then a while later they will evolve into two systems that are quite similar to each other. If one 5-year-old child is 1 cm taller than another 5-year-old child, then ten years late one 15-year old child might be, say, 5cm taller than the other one.

By contrast, in a chaotic system, tens years later one child has grown taller, while the other child has transformed into microscopic octopus. That is not a technical definition of mathematical chaos, but it might give you some idea of what it means.

This video takes another look at chaos. It is called “You can’t script this”, because you can’t predict what will happen with a chaotic system. You can’t write the story, because the most miniscule change in how the story starts might completely change how the story ends.

@[youtube](wqLLm9QN6bk?rel=0)
