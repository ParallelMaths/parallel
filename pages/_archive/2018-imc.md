...
Let `d` be the length of the diagonals of the squares. Each diagonal is the hypotenuse of a right-angled triangle whose other sides are each of length 2. Therefore, by Pythagoras’ Theorem `d^2 = 2^2 + 2^2 = 8`. Hence `d = √8 = 2√2`.  

We therefore see from the figure that the width of the rectangle is `2√2 + √2`, that is, `3√2`. Also, the height of the rectangle is `2√2 + 2√2`, that is, `4√2`.  

Hence the area of the rectangle is given by  

`3√2 × 4√2 = 3 × 4 × √2^2 = 3 × 4 × 2 = 24`.
...
Because each point of the triangle `STU` is at the distance `√3` from the nearest point of the triangle `PQR`, we have `TV = UW = √3`.  

Similarly, the distance from `T` to `PQ` is `√3`. Therefore `T` is equidistant from the sides `QR` and `QP` of the triangle `PQR`.  

Hence the line `QT` bisects the angle of the equilateral triangle at `Q`. Therefore `∠VQT = 30°`.  

It follows that the triangle `TQV` has one right angle and its other angles are `30°` and `60°`. Therefore `QT = 2TV = 2√3`.  

By Pythagoras’ Theorem applied to this triangle, `QT^2 = QV^2 + TV^2`.  

It follows that `QV^2 = QT^2 − TV^2 = (2√3)^2 − (√3)^2 = 12 − 3 = 9`.  
