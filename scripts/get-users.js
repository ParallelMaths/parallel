...
const cleanerPhoneNumber = (phoneNumber) => {
  ...
  const cleaner = phoneNumber.replace(/ /g, '').replace(/\./g, '');
  if(cleaner.length === 10 && cleaner[0] === '7') {
    return `0${cleaner}`;
  }
  if(cleaner.startsWith('447')) {
    return `+${cleaner}`;
  }
  if(cleaner.startsWith('00447')) {
    return `+${cleaner.slice(2)}`;
  }
  return cleaner;
}
